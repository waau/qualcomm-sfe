 ...
 */
static bool sfe_cm_find_dev_and_mac_addr(uint32_t daddr, uint32_t saddr, struct net_device **dev, uint8_t *mac_addr)
{
						  ...
						  int (*okfn)(struct sk_buff *))
{
	 ...
	 */
	if (!sfe_cm_find_dev_and_mac_addr(sic.src_ip, 0, &src_dev, sic.src_mac)) {
		DEBUG_TRACE("failed to find MAC address for src IP: %pI4\n", &sic.src_ip);
		return NF_ACCEPT;
	}

	if (!sfe_cm_find_dev_and_mac_addr(sic.src_ip_xlate, 0, &dev, sic.src_mac_xlate)) {
	...
	if (!sfe_cm_find_dev_and_mac_addr(sic.dest_ip, 0, &dev, sic.dest_mac)) {
	...
	if (!sfe_cm_find_dev_and_mac_addr(sic.dest_ip_xlate, sic.src_ip_xlate, &dest_dev, sic.dest_mac_xlate)) {
